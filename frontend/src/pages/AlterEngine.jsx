...
const HeroThread = () => {
  ...
  const col = (k) => (hov === null || hov === k ? "#5BEA99" : "rgba(255,255,255,.25)");
  return (
    <div data-testid="engine-hero-thread">
      ...
      <svg viewBox="0 0 600 180" className="w-full h-auto" aria-hidden="true">
        ...
        <circle cx="16" cy="90" r="4.5" fill="#5BEA99" />
        ...
        <text x="350" y="48" textAnchor="middle" fontSize="11" fontWeight="500" fill="rgba(159,255,192,.9)" fontFamily="Hanken Grotesk">human gate</text>
      ...
      <div className="flex flex-wrap gap-x-5 gap-y-1 mt-2">
        {HERO_STAGES.map((s) => (
          <button key={s.key} onMouseEnter={() => setHov(s.key)} onMouseLeave={() => setHov(null)} onFocus={() => setHov(s.key)} onBlur={() => setHov(null)} className={`text-[13px] font-medium transition-colors ${hov === s.key ? "text-[#5BEA99]" : "text-white/50"}`}>
            {s.label}
          </button>
        ))}
      </div>
  ...
  return (
    <div className="grid lg:grid-cols-[1.2fr_1fr] gap-12 items-center">
      <svg viewBox="0 0 600 260" className="w-full h-auto" aria-hidden="true">
        ...
        <ThreadPath d="M200 130 L280 130" stroke="#9FFFC0" strokeWidth={2} delay={0.5} duration={0.4} />
        <g>
          <circle cx="294" cy="130" r="13" fill="#0b0b0a" stroke="#9FFFC0" strokeWidth="2" />
          <path d="M294 124 L294 132 M294 136.5 L294 137" stroke="#9FFFC0" strokeWidth="2.2" strokeLinecap="round" />
        </g>
        <text x="294" y="162" textAnchor="middle" fontSize="12" fontWeight="500" fill="rgba(159,255,192,.9)" fontFamily="Hanken Grotesk">step failed</text>
        {BRANCHES.map((b) => (
          <motion.path
            key={b.key}
            d={b.d}
            fill="none"
            stroke={sel === b.key ? "#5BEA99" : "rgba(255,255,255,.2)"}
            strokeWidth={sel === b.key ? 2.4 : 1.6}
            strokeDasharray={b.key === "clarify" ? "5 5" : undefined}
            initial={false}
            animate={{ stroke: sel === b.key ? "#5BEA99" : "rgba(255,255,255,.2)" }}
            transition={{ duration: 0.3, ease: EASE }}
          />
        ))}
        {[[520, 70], [520, 130], [520, 190]].map(([x, y], i) => (
          <circle key={i} cx={x} cy={y} r="6" fill={sel === BRANCHES[i].key ? "#5BEA99" : "rgba(255,255,255,.3)"} />
        ))}
      </svg>
      <div>
        <div className="space-y-1 mb-6">
          {BRANCHES.map((b) => (
            <button
              ...
              className={`block w-full text-left px-4 py-3 border-l-2 transition-colors ${sel === b.key ? "border-[#5BEA99] text-[#fbfaf7] bg-white/[.04]" : "border-white/10 text-white/50"}`}
  ...
  return (
    <>
          ...
          <Reveal delay={0.15}>
            <div className="bg-[#0b0b0a] text-[#fbfaf7] rounded-[6px] border border-black/20 p-7" aria-hidden="true">
              <div className="flex items-center justify-between mb-5">
                ...
                <span className="text-[12px] font-semibold text-[#D8FFE7] flex items-center gap-1.5"><ShieldCheck size={12} aria-hidden="true" /> Needs your decision</span>
              </div>
              <svg viewBox="0 0 420 90" className="w-full h-auto mb-5">
                ...
                <path d="M120 45 L210 45" stroke="#5BEA99" strokeWidth="2.4" fill="none" />
                <rect x="210" y="31" width="28" height="28" transform="rotate(45 224 45)" fill="none" stroke="#5BEA99" strokeWidth="2" />
                ...
                <circle cx="120" cy="45" r="4" fill="#5BEA99" />
              ...
              <div className="flex gap-2">
                <span className="bg-[#5BEA99] text-black text-[12px] font-semibold px-4 py-1.5 rounded-[3px]">Approve</span>
      ...
      <section className="text-[#fbfaf7] py-24 md:py-36 relative overflow-clip" style={{ background: "rgba(0,0,0,0.46)" }} data-testid="engine-composer">
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at 50% 100%, rgba(91,234,153,.12) 0%, rgba(91,234,153,0) 60%)" }} aria-hidden="true" />
