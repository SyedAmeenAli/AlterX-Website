        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 grid md:grid-cols-3 border-t border-l border-white/15">
          {PRICING_SHAPES.map((p, i) => (
            <Reveal key={p.name} delay={i * 0.08}>
              <div className={`border-r border-b border-white/15 p-8 h-full ${i === 1 ? "bg-[#5BEA99]/[.07]" : ""}`} data-testid={`pricing-shape-${p.name.toLowerCase()}`}>
                <p className="text-[12px] font-medium text-[#5BEA99] uppercase tracking-wider mb-3">{String(i + 1).padStart(2, "0")}</p>
                ...
                <ul className="space-y-2.5">
                  {p.points.map((pt) => (
                    <li key={pt} className="flex items-start gap-2.5 text-[14px] text-white/80">
                      <Check size={14} className="text-[#5BEA99] mt-1 shrink-0" aria-hidden="true" /> {pt}
                    </li>
                  ))}
                </ul>
              </div>
