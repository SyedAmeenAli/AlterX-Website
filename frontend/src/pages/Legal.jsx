    ...
    <div className="text-[#090909]" style={{ paddingTop: "calc(var(--header-height) + 56px)", background: "var(--marketing-legal)" }}>
      <div className="max-w-[1200px] mx-auto px-6 md:px-10 pb-24">
        <div className="border-b border-black/15 pb-8 mb-10">
          <p className="ax-eyebrow text-[#123D27] mb-4">Legal</p>
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-5 text-[13px] text-black/55">
            ...
            <span className="text-[11px] font-medium uppercase tracking-[0.16em] border border-[#123D27]/40 text-[#123D27] px-2 py-0.5" data-testid="legal-status">{doc.status}</span>
            <button onClick={() => window.print()} className="inline-flex items-center gap-1.5 font-semibold hover:text-[#123D27] transition-colors" data-testid="legal-print-btn">
          ...
          <nav className="hidden lg:block sticky top-[110px] self-start" aria-label="Document index">
            ...
            <ul className="space-y-2 border-l border-black/15">
              {doc.sections.map((s, i) => (
                <li key={s.h}>
                  <a href={`#${slugify(s.h)}`} className="block pl-4 py-0.5 text-[13px] text-black/60 hover:text-[#123D27] hover:border-l-2 hover:border-[#5BEA99] hover:-ml-[1px] transition-colors">
                    {i + 1}. {s.h}
                  </a>
                </li>
              ))}
