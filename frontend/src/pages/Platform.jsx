  ...
  return (
    <div className="border border-white/15 bg-[#090909]" data-testid="platform-prototype">
      ...
      <div className="flex border-b border-white/12 overflow-x-auto">
        {TABS.map((t, i) => (
          <button key={t} onClick={() => setTab(i)} className={`ax-fill px-5 py-3 text-[13px] font-semibold whitespace-nowrap ${tab === i ? "text-[#5BEA99]" : "text-white/55"}`} data-active={tab === i} data-testid={`platform-tab-${t.toLowerCase()}`}>
            {t}
          </button>
        ))}
      </div>
      <div className="p-6 min-h-[300px]">
        {tab === 0 && (
          <div className="space-y-3 max-w-lg">
            ...
            <div className="border border-[#5BEA99]/30 bg-[#5BEA99]/[.08] px-4 py-3 text-[14px] text-white/90">Understood. Before I draft the plan — should conflicting figures be flagged for review, or excluded entirely?</div>
            <div className="bg-white/[.07] px-4 py-3 text-[14px] text-white/85">Flagged, never averaged.</div>
            <p className="text-[11px] font-medium text-[#5BEA99] pt-2">→ OBJECTIVE CLARIFIED · MOVING TO PLAN</p>
          </div>
        )}
        {tab === 1 && (
          <div className="space-y-2 max-w-lg">
            {[["Collect source extracts", "Data", false], ["Reconcile overlapping figures", "Data", false], ["Draft exception summary", "Documents", false], ["Publish to leadership channel", "Communication", true], ["Archive with sources", "Documents", false]].map(([t, s, ap], i) => (
              <div key={t} className={`flex items-center gap-3 border px-4 py-2.5 ${ap ? "border-[#5BEA99]/50" : "border-white/12"}`}>
                ...
                {ap && <span className="text-[10px] font-medium text-[#D8FFE7]">approval</span>}
              </div>
            ))}
          </div>
        )}
          ...
          <div className="space-y-2 max-w-lg">
            {[["Collect source extracts", "completed"], ["Reconcile overlapping figures", "running"], ["Draft exception summary", "queued"], ["Publish to leadership channel", "waiting approval"], ["Archive with sources", "queued"]].map(([t, st]) => (
              <div key={t} className="flex items-center gap-3 border border-white/12 px-4 py-2.5">
                {st === "completed" ? <Check size={13} className="text-[#5BEA99]" /> : st === "running" ? <CircleDot size={13} className="text-[#9FFFC0] animate-pulse" /> : <Clock size={13} className="text-white/35" />}
                <span className="text-[13px] text-white/85 flex-1">{t}</span>
        ...
        {tab === 3 && (
          <div className="border border-[#5BEA99]/50 p-5 max-w-lg">
            <p className="text-[11px] font-medium text-[#5BEA99] uppercase tracking-wider mb-2">Approval required</p>
            ...
            <div className="flex gap-2">
              <span className="bg-[#5BEA99] text-black text-[12px] font-bold px-4 py-1.5">Approve</span>
        ...
        {tab === 4 && (
          <div className="max-w-lg space-y-3">
            <div className="border border-white/15 p-4">
              <p className="text-[11px] font-medium text-[#5BEA99] uppercase mb-1.5">Result</p>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/12 border border-white/12 mt-20">
            {SECTIONS.map((s, i) => (
              <div key={s.t} className="bg-black p-8" data-testid={`platform-section-${i}`}>
                <p className="text-[12px] font-medium text-[#5BEA99] mb-3">{String(i + 1).padStart(2, "0")}</p>
