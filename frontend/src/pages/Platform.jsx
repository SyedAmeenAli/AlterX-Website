  ...
  return (
    <div className="border border-white/15 bg-[#090909]" data-testid="platform-prototype">
      <div className="flex items-center justify-between border-b border-white/12 px-4 py-2.5">
        <span className="font-mono-ax text-[10px] text-white/50">ALTERX PLATFORM · AX-M-014</span>
        <DemoBadge />
      </div>
      <div className="flex border-b border-white/12 overflow-x-auto">
        {TABS.map((t, i) => (
          <button key={t} onClick={() => setTab(i)} className={`ax-fill px-5 py-3 text-[13px] font-semibold whitespace-nowrap ${tab === i ? "text-[#ff5a1f]" : "text-white/55"}`} data-active={tab === i} data-testid={`platform-tab-${t.toLowerCase()}`}>
            {t}
          </button>
        ))}
      </div>
      <div className="p-6 min-h-[300px]">
        {tab === 0 && (
          <div className="space-y-3 max-w-lg">
            ...
            <div className="border border-[#ff5a1f]/30 bg-[#ff5a1f]/[.08] px-4 py-3 text-[14px] text-white/90">Understood. Before I draft the plan — should conflicting figures be flagged for review, or excluded entirely?</div>
            <div className="bg-white/[.07] px-4 py-3 text-[14px] text-white/85">Flagged, never averaged.</div>
            <p className="font-mono-ax text-[10px] text-[#ff5a1f] pt-2">→ OBJECTIVE CLARIFIED · MOVING TO PLAN</p>
          </div>
        )}
        {tab === 1 && (
          <div className="space-y-2 max-w-lg">
            {[["Collect source extracts", "Data", false], ["Reconcile overlapping figures", "Data", false], ["Draft exception summary", "Documents", false], ["Publish to leadership channel", "Communication", true], ["Archive with sources", "Documents", false]].map(([t, s, ap], i) => (
              <div key={t} className={`flex items-center gap-3 border px-4 py-2.5 ${ap ? "border-[#ff5a1f]/50" : "border-white/12"}`}>
                <span className="font-mono-ax text-[10px] text-white/40">{i + 1}</span>
                <span className="text-[13px] text-white/85 flex-1">{t}</span>
                <span className="text-[11px] text-white/40">{s}</span>
                {ap && <span className="font-mono-ax text-[9px] text-[#ff5a1f]">APPROVAL</span>}
              </div>
            ))}
          </div>
        )}
          ...
          <div className="space-y-2 max-w-lg">
            {[["Collect source extracts", "completed"], ["Reconcile overlapping figures", "running"], ["Draft exception summary", "queued"], ["Publish to leadership channel", "waiting approval"], ["Archive with sources", "queued"]].map(([t, st]) => (
              <div key={t} className="flex items-center gap-3 border border-white/12 px-4 py-2.5">
                {st === "completed" ? <Check size={13} className="text-[#ff5a1f]" /> : st === "running" ? <CircleDot size={13} className="text-[#ff761f] animate-pulse" /> : <Clock size={13} className="text-white/35" />}
                <span className="text-[13px] text-white/85 flex-1">{t}</span>
                <span className="font-mono-ax text-[9px] uppercase text-white/45">{st}</span>
              </div>
            ))}
          </div>
        )}
        {tab === 3 && (
          <div className="border border-[#ff5a1f]/50 p-5 max-w-lg">
            <p className="font-mono-ax text-[10px] text-[#ff5a1f] uppercase tracking-wider mb-2">Approval required</p>
        ...
        {tab === 4 && (
          <div className="max-w-lg space-y-3">
            <div className="border border-white/15 p-4">
              <p className="font-mono-ax text-[10px] text-[#ff5a1f] uppercase mb-1.5">Result</p>
            ...
            <div className="flex gap-2 flex-wrap">
              {["4 checks passed", "1 step revised", "Evidence exportable"].map((c) => (
                <span key={c} className="font-mono-ax text-[10px] text-white/60 border border-white/15 px-2.5 py-1">{c}</span>
              ))}
            </div>
          </div>
        )}
