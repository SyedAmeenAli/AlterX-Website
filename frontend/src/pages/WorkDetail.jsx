  ...
  return (
    <>
      <PageHero eyebrow={w.category} title={w.title} body={w.description}>
        <Reveal delay={0.3}>
          <span className="inline-block mt-8 font-mono-ax text-[10px] uppercase tracking-wider border border-black/30 px-2.5 py-1 text-black/60" data-testid="work-detail-label">{w.label}</span>
        </Reveal>
      </PageHero>
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10">
          <div className="border border-black/15 bg-black text-[#fbfaf7] p-10 md:p-14 mb-16 relative overflow-clip" aria-hidden="true">
            ...
            <p className="relative ax-display text-[16vw] md:text-[9vw] leading-none opacity-90" style={{ fontFamily: "Montserrat, sans-serif", fontWeight: 900 }}>
              {w.name}<span className="text-[#ff5a1f]">.</span>
            </p>
            <p className="relative font-mono-ax text-[10px] uppercase tracking-widest text-white/40 mt-4">Editorial treatment — approved project imagery pending customer review</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-px bg-black/15 border border-black/15 mb-16">
              ...
              <ol className="space-y-3">
                {w.path.map((p, i) => (
                  <li key={p} className="flex gap-3 text-[14px] font-semibold">
                    <span className="font-mono-ax text-[10px] text-[#bd3510] pt-0.5">{String(i + 1).padStart(2, "0")}</span>{p}
                  </li>
                ))}
              </ol>
            </div>
            <div className="bg-[#fbfaf7] p-8">
              ...
              <ul className="space-y-2">{w.systems.map((s) => <li key={s} className="text-[14px] font-semibold border-l-2 border-[#ff5a1f] pl-3">{s}</li>)}</ul>
            </div>
            <div className="bg-[#fbfaf7] p-8">
