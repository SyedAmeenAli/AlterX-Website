...
export default function Approvals() {
  ...
  return (
    <div className="p-6 md:p-10 max-w-[1000px]" data-testid="approvals-page">
      ...
      <h2 className="font-mono-ax text-[10px] uppercase tracking-wider text-[#ff5a1f] mb-3">Waiting for a decision</h2>
      {pending.length === 0 ? (
        ...
        <div className="space-y-3 mb-10">
          {pending.map((m) => {
            ...
            return (
              <Link key={m.id} to={`/try-alter-engine/missions/${m.id}`} className="block border border-[#ff5a1f]/50 p-5 hover:bg-white/[.03] transition-colors" data-testid={`approval-pending-${m.id}`}>
                <p className="flex items-center gap-2 font-mono-ax text-[10px] uppercase tracking-wider text-[#ff5a1f] mb-2"><ShieldCheck size={12} aria-hidden="true" /> {m.id} · Approval required</p>
      ...
      <h2 className="font-mono-ax text-[10px] uppercase tracking-wider text-white/45 mb-3">Decision history</h2>
      {decisions.length === 0 ? (
        ...
        <div className="border border-white/12 divide-y divide-white/10">
          {decisions.map((d, i) => (
            <div key={i} className="flex flex-wrap items-center gap-3 px-4 py-3" data-testid={`decision-row-${i}`}>
              <Link to={`/try-alter-engine/missions/${d.missionId}`} className="font-mono-ax text-[11px] text-white/45 hover:text-[#ff5a1f]">{d.missionId}</Link>
              <span className={`font-mono-ax text-[10px] uppercase px-2 py-0.5 border ${d.decision === "declined" ? "text-[#ff8a63] border-[#bd3510]/50" : "text-[#ff5a1f] border-[#ff5a1f]/40"}`}>{d.decision}</span>
