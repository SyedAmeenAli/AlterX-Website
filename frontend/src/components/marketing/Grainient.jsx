...
/*
  Grainient — one OGL/WebGL renderer, uniform-driven. Architecture kept as
  supplied: build once (Effect 1), sync props to uniforms only (Effect 2),
  never rebuild the Program on prop change.

  `paused` (new): render exactly one frame and never start the RAF loop —
  used for prefers-reduced-motion so the same orange/off-white/black
  appearance renders without continuous animation.
  `maxDpr` (new): caller-controlled device-pixel-ratio ceiling (mobile
  wants a lower cap than desktop).
*/
const Grainient = ({
  timeSpeed = 0.25,
  colorBalance = 0.0,
  ...
  useEffect(() => {
    ...
    const ctx = { renderer, program, mesh };
    ...
    const loop = (t) => {
      ...
      renderer.render({ scene: mesh });
      if (pausedRef.current) { raf = 0; return; } // one settled frame, then stop
      raf = requestAnimationFrame(loop);
    };

    const tryStart = () => {
      if (pausedRef.current) { renderer.render({ scene: mesh }); return; } // static frame
  ...
  // paused toggling after mount: reuse the same start/stop the mount
  // effect already wired up — no second RAF loop is ever created.
  useEffect(() => {
    const container = containerRef.current;
    const ctx = container && ctxMap.get(container);
    if (!ctx) return;
    if (paused) { ctx.tryStop?.(); ctx.renderer.render({ scene: ctx.mesh }); }
    else ctx.tryStart?.();
  }, [paused]);
  ...
  useEffect(() => {
    ...
    if (paused) renderer.render({ scene: mesh }); // reflect prop edits immediately while static
  }, [
    timeSpeed, colorBalance, warpStrength, warpFrequency, warpSpeed,
    warpAmplitude, blendAngle, blendSoftness, rotationAmount, noiseScale,
    grainAmount, grainScale, grainAnimated, contrast, gamma, saturation,
    centerX, centerY, zoom, color1, color2, color3, paused,
  ]);
